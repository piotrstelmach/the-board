...
const USERS_PAGINATED_CACHE_NAME = 'pagination:user';
...
export const getAllUsers = async (
  ...
  try {
    const cache = await redisClient.hGetAll(
      `${USERS_PAGINATED_CACHE_NAME}:page${page}limit:${limit}`
    );
...
export const createNewUser = async (
  ...
  try {
    await invalidatePaginatedCache(USERS_PAGINATED_CACHE_NAME);
